...
export const COOKIE_BANNER_VISIBILITY_EVENT = 'kgk:cookie-banner-visibility';
...
export const analyticsCookiePreferences = (): CookieConsentPreferences => ({
  necessary: true,
  functionality: true,
  analytics: true,
  consentedAt: new Date().toISOString(),
...
// Alias for allCookiePreferences used in cookie_banner
export const allCookiePreferences = analyticsCookiePreferences;
...
export const notifyCookieBannerVisibility = (visible: boolean) => {
  if (typeof window === 'undefined') {
    return;
  }
